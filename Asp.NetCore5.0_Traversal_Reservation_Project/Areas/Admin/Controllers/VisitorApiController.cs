...
namespace Asp.NetCore5._0_Traversal_Reservation_Project.Areas.Admin.Controllers
{
    ...
    public class VisitorApiController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            var responseMessage = await client.GetAsync("http://localhost:41981/api/Visitor");
        ...
        public async Task<IActionResult> AddVisitor(VisitorViewModel visitorViewModel)
        {
            ...
            var responseMessage = await client.PostAsync("http://localhost:41981/api/Visitor",content);
            if(responseMessage.IsSuccessStatusCode)
            {
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.DeleteAsync($"http://localhost:41981/api/Visitor/{id}");
            if(responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index");
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.GetAsync($"http://localhost:41981/api/Visitor/{id}");
            if (responseMessage.IsSuccessStatusCode)
            {
                var jsonData = await responseMessage.Content.ReadAsStringAsync();
        ...
        public async Task<IActionResult> UpdateVisitor(VisitorViewModel visitorViewModel)
        {
            ...
            var jsonData = JsonConvert.SerializeObject(visitorViewModel);
            StringContent content = new(jsonData, Encoding.UTF8, "application/json");
            var responseMessage = await client.PutAsync("http://localhost:41981/api/Visitor", content);
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index");
